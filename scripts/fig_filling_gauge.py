...
in green, others in purple.  A vertical dashed line marks Pi_max and
the gap to 1.0 is labelled H_Pi.
"""
import numpy as np
...
def main():
    ...
    ax.set_yticklabels(SPC_CATEGORIES)
    ax.set_xlabel(r"Possibility $\pi(\omega)$")
    ax.set_xlim(0, 1.12)
    ax.set_ylim(-0.5, SPC_N - 0.2)
    ...
    # Legend
    legend_elements = [
        mpatches.Patch(facecolor=GREEN, edgecolor="white", label="Observed (MDT)"),
