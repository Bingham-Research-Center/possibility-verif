...
hedged-correct, sharp-wrong) using convective mode categories.
...
# ---------------------------------------------------------------------------
# Scenario definitions (K=5: NULL, CELL, SUPER, QLCS, MCS)
# ---------------------------------------------------------------------------
SCENARIOS = {
    "Sharp-Correct": {
    ...
    "Hedged-Correct": {
        "pi": np.array([0.05, 0.10, 0.20, 0.40, 0.55]),
        "obs": "MCS",
    },
    "Sharp-Wrong": {
        "pi": np.array([0.80, 0.10, 0.05, 0.00, 0.00]),
        "obs": "SUPER",
    },
}
...
def compute_scorecard(pi, obs_category):
    ...
    Parameters
    ----------
    pi : array-like, shape (CONV_N,)
        Raw possibility values for each convective mode category.
    obs_category : str
        Observed convective mode label (must be in CONV_MODES).
    ...
    obs_idx = CONV_MODES.index(obs_category)
    ...
    mask = np.ones(CONV_N, dtype=bool)
    mask[obs_idx] = False
...
def main():
    ...
    for ax, (title, scenario) in zip(axes, SCENARIOS.items()):
        ...
        colours = [GREEN if i == obs_idx else PURPLE for i in range(CONV_N)]
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(CONV_MODES, fontsize=8)
