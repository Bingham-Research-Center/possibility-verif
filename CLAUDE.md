...
- **Figures**: `python scripts/generate_all.py` (8 figs), then separately `python scripts/fig_scorecard_table.py`, `python scripts/fig_performance_diagram.py`, `python scripts/fig_severity_matrix.py`, `python scripts/fig_contingency_schematic.py`. Outputs 13 manuscript PNGs to `figures/`.
...
## Figures (13 in manuscript, all PNG)
...
| `ig_decomposition` | `fig_ig_decomposition.py` | §3.2 | Overlay bar IG decomposition (DSC, REL) |
| `verification_lanes` | `fig_verification_lanes.py` | §5.2 | Three-facet flowchart schematic |
...
| `contingency_schematic` | `fig_contingency_schematic.py` | §4.4 | Annotated 2×2 table + K×K extension with POD/FAR/CSI/PSS/HSS |
...
Synthetic reforecast data generated by `fig_performance_diagram.py:generate_reforecast(n_days=800)` — shared by hexbin, commitment, categorical-scores, and reliability-curves figures.

## Architecture

The paper has three verification facets:
- **Categorical facet** (§4, §5, §6): mode-based threshold verification — POD, FAR, CSI, PSS, HSS per threshold / full K×K table. No distributional conversion needed. Definitions in §4.4.
- **Probabilistic facet** (§3): IG via possibility-to-probability conversion — converts to probabilities (with explicit ignorance outcome), measures information gain relative to SPC climatological baseline.
- **Native possibilistic facet** (§4): five-number scorecard — evaluates shape and confidence without conversion (α*, η, δ, H_Π, N_c*).
...
- Round 2 revision complete (see `reviews/ROUND2-RESPONSE.md`):
  ...
  - Deterministic lane → categorical facet with POD/FAR/CSI/HSS (M6)
...
- Round 3 editorial rewrite (Overleaf, April 2026):
  - All section prose rewritten (§2–§8) for clarity and concision
  - All 7 MISSING citation keys resolved with Paperpile entries
  - Categorical verification scores (POD/FAR/CSI/PSS/HSS) moved to new §4.4 with formal definitions
  - New Figure 13: contingency schematic (`fig_contingency_schematic.py`)
  - Backmatter partially filled (funding line, expanded GenAI statement)
  - All .tex files reflowed to semantic line breaks (~70 chars)

### Still needed
- Data Availability: needs version/release tag and Zenodo DOI
- Scattered `(REF)` / `(REFS)` placeholders in prose (§1, §3) need specific citation keys or removal
