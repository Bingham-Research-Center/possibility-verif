...
Convective mode prediction---classifying storms as discrete cells,
supercells, quasi-linear convective systems (QLCS), or mesoscale
convective systems (MCS)---offers a natural running example.  Mode
categories are point-verifiable (an observer can identify the storm
type), genuinely independent (a supercell is not `one step worse' than a
cell), and inherently uncertain (mode evolution depends on mesoscale
processes that are poorly resolved by current models).
...
Possibility theory generalizes probability: the interval
...
possibility--necessity duality.  In convective mode prediction, a
forecaster may wish to express that \mode{SUPER} is plausible but not
...
$\pi(\mode{SUPER}) = 0.7$ says ``\mode{SUPER} is fairly compatible with
available evidence''; it does not say ``the long-run frequency of
\mode{SUPER} events is 70\%.''
...
signal, a problem we call \textit{ignorance erasure}: it conflates ``the
system thinks \mode{QLCS} is most likely'' with ``the system is
...
(reliability, resolution, discrimination) that we adapt below.
...
probability may not capture an agent's epistemic state, but none
provides a verification methodology for subnormal possibility
distributions (Section~\ref{sec:ds_connections} develops these
connections).
...
verification: the five-number scorecard and interval log score.
Appendix~\ref{sec:appendix_alt} develops the imprecise ranked
logarithmic score and possibilistic climatology.
...
using convective mode categories (\mode{NULL} through \mode{MCS}).
