...
Possibility theory extends probability: the interval
$[\nec(A),\, \poss(A)]$ brackets the range of consistent probabilities,
collapsing to a point when evidence is complete
(Section~\ref{sec:poss_axioms}).  Possibility distributions arise naturally in several operational
...
Normalizing a subnormal distribution to $\max(\pi) = 1$ erases this
signal, a problem termed \textit{ignorance erasure}: it conflates ``the
system thinks \spc{ENH} is most likely'' with ``the system is
confident'' (Section~\ref{sec:naive_normalization} develops this
contrast in detail).
...
possibilistic predictions lack a dedicated verification methodology.
Existing approaches either normalize the distribution---
...
possibility from probability.  This paper presents a framework
with three components: a five-number scorecard for possibilistic
evaluation, a possibility-to-probability bridge that preserves ignorance
as an explicit outcome, and a categorical verification lane using
threshold-based categorical scores familiar to operational meteorologists.
